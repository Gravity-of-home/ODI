...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.CONNECT == accessor.getCommand()) { // websocket 연결요청
            ...
            chatService.sendChatMessage(ChatMessageDTO.builder().roomId(roomId).senderNickname(nickname).type(MessageType.ENTER).build());
            ...
            String nickname = Optional.ofNullable((Principal) message.getHeaders().get("simpUser")).map(Principal::getName).orElse("UnknownUser");
            chatService.sendChatMessage(ChatMessageDTO.builder().roomId(roomId).senderNickname(nickname).type(MessageType.QUIT).build());
